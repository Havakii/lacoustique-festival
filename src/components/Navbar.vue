<template>
  <div class="app">
    <div class="!bg-[#7AD3FF] sm:!bg-transparent fixed w-full z-50 navbar">

      <div class="flex items-center justify-between py-1 z-50">
        <div v-if="!darkMode">
        ...
        <h3 :class="darkMode
    ? 'sm:hidden md:hidden lg:hidden font-semibold pl-10 text-start text-2xl drop-shadow-md text-white title-logo'
    : 'sm:hidden md:hidden lg:hidden font-semibold pl-10 text-start text-2xl drop-shadow-md text-black title-logo'">
          Lacoustique
        </h3>
        ...
        <button class="sm:hidden cursor-pointer" @click="toggleNav">
          ...
          <svg v-if="showNav" xmlns="http://www.w3.org/2000/svg"
            :class="darkMode ? 'h-7 w-7 mr-3 text-white' : 'h-7 w-7 mr-3 text-black'" fill="none" viewBox="0 0 24 24"
      ...
      <div class="bg-[#7AD3FF] z-5 w-screen" :class="{ hidden: !showNav, absolute: showNav }">
